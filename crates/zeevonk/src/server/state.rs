...
mod show_data_builder {
    ...
    pub(crate) fn build_from_showfile(showfile: &Showfile) -> Result<ShowData, server::Error> {
        ...
        for fixture in showfile.patch().fixtures() {
            ...
            let dmx_mode =
                fixture_type.dmx_mode(fixture.kind().gdtf_dmx_mode()).ok_or_else(|| {
    ...
    impl<'a> FixtureBuilder<'a> {
        ...
        fn collect_child_fixtures(
            ...
            for child_geometry in geometry.children() {
                ...
                let fixtures_for_child =
                    self.fixtures_from_geometry(sub_fixture_path, child_geometry);
        ...
        fn resolve_virtual_channels(&mut self) {
            ...
            for (cf_id, virtual_attribute) in &self.unresolved_virtual_channels {
                ...
                let Some(fixture) =
                    self.fixtures.iter_mut().find(|f| f.path() == cf_id.fixture_path)
                else {
                    continue;
                };
        ...
        fn get_relations_for_dmx_channel(
            ...
            for relation in relations {
                let Some((_, _, follower_channel_function)) =
