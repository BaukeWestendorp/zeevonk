...
pub fn dump_patch(project_path: PathBuf) -> anyhow::Result<()> {
    ...
    let mut sorted_fixtures: Vec<&Fixture> =
        zeevonk.project().stage().fixtures().values().collect();
    sorted_fixtures.sort_by_key(|f| f.id());
...
mod dump {
    ...
    fn dump_fixture_with_depth(fixture: &Fixture, _depth: usize) {
        ...
        if id.len() == 1 {
            ...
            println!(
                "{guide}{BOLD}{MAGENTA}{name}{RESET} {DIM}({RESET}id{DIM}={RESET}{YELLOW}{id}{RESET}{DIM}, {RESET}base_addr{DIM}={RESET}{YELLOW}{base_address}{RESET}{DIM}){RESET}"
            );
            ...
            println!(
                "{guide}{BOLD}{MAGENTA}{name}{RESET} {DIM}({RESET}id{DIM}={RESET}{YELLOW}{id}{RESET}{DIM}){RESET}"
            );
        }
            ...
            for (attribute, fun) in channels {
                let kind = match fun.kind() {
                    ...
                    FixtureChannelFunctionKind::Virtual { relations } => relations
                        .iter()
                        .map(|relation| {
                            format!("{}->{}", relation.fixture_id(), relation.attribute())
                        })
                        .collect::<Vec<_>>()
                        .join(", "),
                };
