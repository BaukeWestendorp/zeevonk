...
impl EnttecOpenDmxOutput {
    pub fn new(
        universe_id: UniverseId,
        serial_number: String,
    ) -> Result<Self, crate::output::Error> {
        Ok(Self {
            universe_id,
            serial_number,
...
fn ftdi_init(ftdi: &mut Ftdi) -> Result<(), crate::output::Error> {
    ftdi.reset()?;
...
fn ftdi_send(ftdi: &mut Ftdi, buffer: &[u8]) -> Result<(), crate::output::Error> {
    ftdi.set_break_on()?;
    ftdi.set_break_off()?;
...
fn ftdi_close(ftdi: &mut Ftdi) -> Result<(), crate::output::Error> {
    ftdi.close()?;
    Ok(())
}
